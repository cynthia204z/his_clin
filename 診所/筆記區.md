...
- 次用途類別 (xmlSubTypeNcid) 和目錄名稱 (xmlName) 的對應關係？
...
​	  	==從外部取得用途類別 xmlTypeNcid (醫師/科別--常用/套餐)== + ==使用單元別 ownerNcid (醫師/科別代碼)==

​	=>  ==從下拉取得次用途類別 xmlSubTypeNcid (ICD10、PKG、EXAM、NOTE...)==

​	=>  ==產生頂級目錄名稱 xmlName== `EX: "個人常用(" + this.xmlName + ")"` + ==使用分類 fillerSysNcid==
...
​	先在 datatreeList 裡找有沒有資料的 xmlSubTypeNcid 和下拉的綁定值(type) 相同，有的話就把datatree裡的資料換成該筆資料；

​	沒有就新增到 datatreeList ，並把datatree裡的資料換成新增的資料
